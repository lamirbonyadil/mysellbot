 ...
 * further down. Deliberately duplicated from cronvolume.php's version
 * (rather than sharing a helper) since it extracts different fields
 * (expire, not data_limit/used_traffic) and each cron file runs as its
 * own standalone process/request, so there's no namespace collision risk.
 *
 ...
 * @return array<string, array{status:string, expire:?int}>
