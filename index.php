...
if (!in_array($Chat_type, ["private"]))
    return;
#-----------telegram_ip_ranges------------#
$is_local = in_array($_SERVER['REMOTE_ADDR'], ['127.0.0.1', '::1']) || isset($_SERVER['HTTP_X_FORWARDED_FOR']);
if (!$is_local && !checktelegramip()) {
...
function buildInvoiceContext(array $user): string
{
...
if (strpos($text, "/start ") !== false) {
    ...
    if (ctype_digit($affiliatesid)) {
        ...
        $text_start = "👋 سلام <b>{$first_name}</b> عزیز\nخوش آمدید 🌟";
...
#-----------/status (my packages)------------#
if ($text == "/status") {
    $stmt = $pdo->prepare("SELECT * FROM invoice WHERE id_user = :id_user AND (status = 'active' OR status = 'end_of_time'  OR status = 'end_of_volume' OR status = 'sendedwarn' OR status = 'disabled')");
    $stmt->bindParam(':id_user', $from_id);
    $stmt->execute();
    ...
    $stmt = $pdo->prepare("SELECT * FROM invoice WHERE id_user = :id_user AND (status = 'active' OR status = 'end_of_time'  OR status = 'end_of_volume' OR status = 'sendedwarn' OR status = 'disabled') ORDER BY time_sell DESC LIMIT $start_index, $items_per_page");
    $stmt->bindParam(':id_user', $from_id);
    $stmt->execute();
    $keyboardlists = [
        'inline_keyboard' => [],
...
if (!function_exists('sendBotUserNotFoundError')) {
    function sendBotUserNotFoundError($from_id, $textbotlang, $message_id = null)
    {
        if ($message_id) {
            deletemessage($from_id, $message_id);
        }
        $error_text = "❌ <b>نام کاربری وجود ندارد!</b>\n\n" .
            "این مورد می‌تواند به دو دلیل باشد:\n" .
            "1️⃣ نام کاربری را اشتباه وارد کرده‌اید و مایلید دوباره تلاش کنید.\n" .
            "2️⃣ هنوز سرویسی تهیه نکرده‌اید یا اکانتی ندارید.";

        $error_keyboard = json_encode([
            'inline_keyboard' => [
                [['text' => '🔄 تلاش مجدد (ورود نام کاربری)', 'callback_data' => 'usernotlist']],
                [['text' => '🛍️ سرویس ندارید؟ برای خرید کلیک کنید', 'callback_data' => 'buy']],
                [['text' => '🔙 بازگشت به منوی اصلی', 'callback_data' => 'backuser']]
            ]
        ]);
        sendmessage($from_id, $error_text, $error_keyboard, 'html');
        step('home', $from_id);
    }
}
if (!function_exists('sendBotUserAlert')) {
    function sendBotUserAlert($from_id, $type = 'not_found', $message_id = null)
    {
        if ($message_id) {
            deletemessage($from_id, $message_id);
        }

        // تفکیک متون بر اساس نوع خطا جهت جلوگیری از هاردکد چندباره در بدنه کد
        if ($type === 'already_taken') {
            $text = "❌ این نام کاربری قبلاً به حساب کاربری شخص دیگری در ربات متصل شده است.\n\n💡 اگر این سرویس متعلق به شماست، لطفاً با پشتیبانی در ارتباط باشید، در غیر این صورت می‌توانید یک سرویس جدید برای خود تهیه کنید.";
            $buttons = [
                [['text' => '🛍️ خرید سرویس جدید', 'callback_data' => 'buy']],
                [['text' => '🔄 تلاش مجدد (نام کاربری دیگر)', 'callback_data' => 'usernotlist']],
                [['text' => '📞 پیام به پشتیبانی', 'callback_data' => 'support']]
            ];
        } else {
            $text = "❌ <b>نام کاربری وجود ندارد!</b>\n\nاین مورد می‌تواند به دو دلیل باشد:\n1️⃣ نام کاربری را اشتباه وارد کرده‌اید و مایلید دوباره تلاش کنید.\n2️⃣ هنوز سرویسی تهیه نکرده‌اید یا اکانتی ندارید.";
            $buttons = [
                [['text' => '🔄 تلاش مجدد (ورود نام کاربری)', 'callback_data' => 'usernotlist']],
                [['text' => '🛍️ سرویس ندارید؟ برای خرید کلیک کنید', 'callback_data' => 'buy']]
            ];
        ...
        $buttons[] = [['text' => '🔙 بازگشت به منوی اصلی', 'callback_data' => 'backuser']];

        sendmessage($from_id, $text, json_encode(['inline_keyboard' => $buttons]), 'html');
        step('home', $from_id);
    }
}

if (!function_exists('renderBotUserServiceInfo')) {
    function renderBotUserServiceInfo($from_id, $DataUserOut, $panel, $textbotlang, $message_id = null)
    {
        if ($message_id) {
            deletemessage($from_id, $message_id);
        }
        ...
        $output          = ($DataUserOut['data_limit'] ?? 0) - ($DataUserOut['used_traffic'] ?? 0);
        ...
        $timeDiff        = ($DataUserOut['expire'] ?? 0) - time();
        $day             = $DataUserOut['expire'] ? floor($timeDiff / 86400) + 1 . $textbotlang['users']['status']['day'] : $textbotlang['users']['status']['Unlimited'];

        $text_info = "<b>" . $textbotlang['users']['status']['info'] . "</b>\n\n" .
            "👤 " . $textbotlang['users']['status']['username'] . " <code>" . $DataUserOut['username'] . "</code>\n" .
            "📍 لوکیشن سرویس: <b>" . $panel['name_panel'] . "</b>\n" .
            "⚡ " . $textbotlang['users']['status']['status'] . " " . $status_var . "\n" .
            "📅 " . $textbotlang['users']['status']['expirationDate'] . " " . $expirationDate . "\n" .
            "⏳ " . $textbotlang['users']['status']['daysleft'] . " " . $day . "\n" .
            "📦 " . $textbotlang['users']['status']['LastTraffic'] . " " . $LastTraffic . "\n" .
            "📉 " . $textbotlang['users']['status']['usedTrafficGb'] . " " . $usedTrafficGb . "\n" .
            "🔋 " . $textbotlang['users']['status']['RemainingVolume'] . " " . $RemainingVolume . "\n";

        $keyboardinfo = json_encode([
            'inline_keyboard' => [[['text' => '➕ افزودن سرویس به حساب من', 'callback_data' => "binduser_" . $panel['id'] . "_" . $DataUserOut['username']]]]
        ]);

        sendmessage($from_id, $text_info, $keyboardinfo, 'html');
        step('home', $from_id);
    }
}
if ($user['step'] == "getusernameinfo") {
    ...
    $stmt_check_bound = $pdo->prepare("SELECT 1 FROM invoice WHERE username = :username AND (status = 'active' OR status = 'end_of_time' OR status = 'end_of_volume' OR status = 'sendedwarn') LIMIT 1");
    $stmt_check_bound->execute([':username' => $text]);

    if ($stmt_check_bound->fetchColumn()) {
        sendBotUserAlert($from_id, 'already_taken');
        return;
    }

    $loading_msg = sendmessage($from_id, "⏳ در حال جستجوی نام کاربری شما ... لطفا کمی صبر کنید.", null, 'html');
    $loading_id = $loading_msg['result']['message_id'] ?? null;

    $stmt_panels = $pdo->prepare("SELECT * FROM marzban_panel WHERE status = 'activepanel'");
    $stmt_panels->execute();

    $found_user = false;
    while ($panel = $stmt_panels->fetch(PDO::FETCH_ASSOC)) {
        $DataUserOut = $ManagePanel->DataUser($panel['name_panel'], $text);
        if ($DataUserOut && (!isset($DataUserOut['status']) || $DataUserOut['status'] != "Unsuccessful")) {
            $found_user = true;
            if ($loading_id) deletemessage($from_id, $loading_id);
            renderBotUserServiceInfo($from_id, $DataUserOut, $panel, $textbotlang);
            break;
        }
    }
    ...
    if (!$found_user) {
        if ($loading_id) deletemessage($from_id, $loading_id);
        sendBotUserAlert($from_id, 'not_found');
    }
    return;
} elseif (preg_match('/locationnotuser_(.*)/', $datain, $dataget)) {
    $panel = select("marzban_panel", "*", "id", $dataget[1], "select");
    if (!$panel) return;

    $DataUserOut = $ManagePanel->DataUser($panel['name_panel'], $user['Processing_value']);
    if (!$DataUserOut || (isset($DataUserOut['status']) && $DataUserOut['status'] == "Unsuccessful")) {
        sendBotUserAlert($from_id, 'not_found', $message_id);
        return;
    }

    renderBotUserServiceInfo($from_id, $DataUserOut, $panel, $textbotlang, $message_id);
} elseif (preg_match('/binduser_(\d+)_([a-zA-Z0-9_.-]+)/', $datain, $dataget)) {
    $locationid = $dataget[1];
    $username_ac = $dataget[2];

    // Find panel configuration details
    $marzban_list_get = select("marzban_panel", "*", "id", $locationid, "select");
    if ($marzban_list_get == false) {
        sendmessage($from_id, "پنل مورد نظر یافت نشد.", null, 'html');
        return;
    }

    // Guard Clause: Prevent adding a service that's already bound in the database
    $stmt_check = $pdo->prepare("SELECT 1 FROM invoice WHERE username = :username AND Service_location = :loc");
    $stmt_check->execute([':username' => $username_ac, ':loc' => $marzban_list_get['name_panel']]);
    if ($stmt_check->fetchColumn()) {
        telegram('answerCallbackQuery', [
            'callback_query_id' => $callback_query_id,
            'text' => "این سرویس قبلاً به حساب کاربری شما یا شخص دیگری متصل شده است.",
            'show_alert' => true,
        ]);
        return;
    }

    // Pull accurate current specifications directly from the VPN server API
    $DataUserOut = $ManagePanel->DataUser($marzban_list_get['name_panel'], $username_ac);
    $volume_gb = $DataUserOut['data_limit'] ? round($DataUserOut['data_limit'] / pow(1024, 3)) : 0;
    $days_left = $DataUserOut['expire'] ? max(0, floor(($DataUserOut['expire'] - time()) / 86400)) : 0;

    // Fetch product information based on volume match and panel location
    $stmt_product = $pdo->prepare("SELECT * FROM product WHERE Volume_constraint = :volume AND (location = :loc OR location = '/all') ORDER BY location DESC LIMIT 1");
    $stmt_product->execute([
        ':volume' => $volume_gb,
        ':loc' => $marzban_list_get['name_panel']
    ]);
    $product_info = $stmt_product->fetch(PDO::FETCH_ASSOC);

    if ($product_info) {
        $product_name = $product_info['name_product'];
        $price = $product_info['price_product'];
    } else {
        // Fallback default info if no matching product package exists in database
        $product_name = "افزودن دستی (" . $volume_gb . " گیگ)";
        $price = "0";
    }
    ...
    $date = time();
    $Status = "active";
    $Volume_warning_level = 0;
    $last_inactivity_notify = 0;

    // Bind this item permanently to the user inside database
    $sql = "INSERT IGNORE INTO invoice (id_user, id_invoice, username, time_sell, Service_location, name_product, price_product, Volume, Service_time, Status, Volume_Warning_Level, last_inactivity_notify) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(1, $from_id);
    $stmt->bindParam(2, $randomString);
    $stmt->bindParam(3, $username_ac, PDO::PARAM_STR);
    $stmt->bindParam(4, $date);
    $stmt->bindParam(5, $marzban_list_get['name_panel'], PDO::PARAM_STR);
    $stmt->bindParam(6, $product_name, PDO::PARAM_STR);
    $stmt->bindParam(7, $price);
    $stmt->bindParam(8, $volume_gb);
    $stmt->bindParam(9, $days_left);
    $stmt->bindParam(10, $Status);
    $stmt->bindParam(11, $Volume_warning_level);
    $stmt->bindParam(12, $last_inactivity_notify);
    $stmt->execute();

    if (isset($setting['Channel_Report']) && strlen($setting['Channel_Report']) > 0) {
        $text_report = "➕ <b>افزودن سرویس دستی</b>\n\n"
            . "👤 کاربر: <a href='tg://user?id={$from_id}'>{$first_name}</a> | <code>{$from_id}</code>\n"
            . "🔖 یوزرنیم تلگرام: @{$username}\n"
            . "💊 سرویس: <code>{$username_ac}</code>\n"
            . "📍 لوکیشن: <b>{$marzban_list_get['name_panel']}</b>\n"
            . "📦 پکیج: <b>{$product_name}</b>\n"
            . "🗓 تاریخ: " . date('Y/m/d H:i:s');
        sendmessage($setting['Channel_Report'], $text_report, null, 'HTML');
    }
    // Trigger success notification banner inside Telegram
    telegram('answerCallbackQuery', [
        'callback_query_id' => $callback_query_id,
    ...
    // Smoothly redirect and display the user's updated active packages list (Mirza /status wrapper)
    update("user", "pagenumber", "1", "id", $from_id);
    $page = 1;
    $items_per_page = 10;
    $start_index = ($page - 1) * $items_per_page;
    ...
    $usernotlist = [['text' => $textbotlang['Admin']['Status']['notusenameinbot'], 'callback_data' => 'usernotlist']];
    $pagination_buttons = [
        ['text' => $textbotlang['users']['page']['next'], 'callback_data' => 'next_page'],
        ['text' => $textbotlang['users']['page']['previous'], 'callback_data' => 'previous_page']
    ];
    ...
    $keyboardlists['inline_keyboard'][] = $pagination_buttons;
    $keyboardlists['inline_keyboard'][] = [['text' => $textbotlang['users']['backhome'], 'callback_data' => "backuser"]];
    $keyboard_json = json_encode($keyboardlists);
...
if (preg_match('/delete_orphan_([a-zA-Z0-9_.-]+)/', $datain, $dataget)) {
    $id_invoice = $dataget[1];
    $invoice_row = select("invoice", "*", "id_invoice", $id_invoice, "select");
    if ($invoice_row == false || $invoice_row['id_user'] != $from_id) {
        telegram('answerCallbackQuery', [
            'callback_query_id' => $callback_query_id,
            'text'              => '❌ سرویس یافت نشد.',
            'show_alert'        => true,
    ...
    $stmt = $pdo->prepare("DELETE FROM invoice WHERE id_invoice = :id_invoice AND id_user = :id_user");
    $stmt->bindParam(':id_invoice', $id_invoice);
    $stmt->bindParam(':id_user',    $from_id);
    $stmt->execute();

    // Rebuild and show the services list in the same message
    update("user", "pagenumber", "1", "id", $from_id);
    $stmt = $pdo->prepare("SELECT * FROM invoice WHERE id_user = :id_user AND (status = 'active' OR status = 'end_of_time' OR status = 'end_of_volume' OR status = 'sendedwarn') ORDER BY time_sell DESC LIMIT 10");
    $stmt->bindParam(':id_user', $from_id);
    $stmt->execute();
    ...
    Editmessagetext($from_id, $message_id, $textbotlang['users']['sell']['service_sell'], json_encode($keyboardlists));
}
if (preg_match('/product_([a-zA-Z0-9_.-]+)/', $datain, $dataget)) {
    ...
    // if (isset($DataUserOut['msg']) && $DataUserOut['msg'] == "User not found") {
    //     update("invoice", "Status", "disabledn", "id_invoice", $nameloc['id_invoice']);
    //     $keyboard_notfound = json_encode([
    //         'inline_keyboard' => [
    //             [
    //                 ['text' => '🗑 حذف این سرویس از لیست من', 'callback_data' => 'delete_orphan_' . $nameloc['id_invoice']],
    //             ],
    //             [
    ...
    //         ]
    //     ]);
    //     Editmessagetext($from_id, $message_id, $textbotlang['users']['status']['usernotfound'], $keyboard_notfound);
    //     return;
    // }
    ...
    $isNotFound = !$DataUserOut
        || (isset($DataUserOut['msg']) && $DataUserOut['msg'] == "User not found")
        || empty($DataUserOut['status'])
        || !isset($DataUserOut['status']);

    $isError = !$isNotFound && $DataUserOut['status'] == "Unsuccessful";
...
if (preg_match('/subscriptionurl_([a-zA-Z0-9_.-]+)/', $datain, $dataget)) {
    $username = $dataget[1];
    $nameloc = select("invoice", "*", "username", $username, "select");
    $marzban_list_get = select("marzban_panel", "*", "name_panel", $nameloc['Service_location'], "select");
    $DataUserOut = $ManagePanel->DataUser($nameloc['Service_location'], $username);
    ...
    unlink($urlimage);
} elseif (preg_match('/config_([a-zA-Z0-9_.-]+)/', $datain, $dataget)) {
    $username = $dataget[1];
    $nameloc = select("invoice", "*", "username", $username, "select");
    $marzban_list_get = select("marzban_panel", "*", "name_panel", $nameloc['Service_location'], "select");
    $DataUserOut = $ManagePanel->DataUser($nameloc['Service_location'], $username);
    ...
    // user (in serviceextendselect_) that this remaining volume/time will be
    // lost when the renewal is applied.
    $panelSnapshot = json_encode([
        'expire'       => $DataUserOut['expire'] ?? null,
        'data_limit'   => $DataUserOut['data_limit'] ?? null,
        'used_traffic' => $DataUserOut['used_traffic'] ?? null,
    ]);
    update("user", "Processing_value_three", $panelSnapshot, "id", $from_id);
    ...
    // If the user still has meaningful volume/time left on the current plan,
    // stop here and make them explicitly confirm, since renewing will wipe
    // it out and it can't be carried over to the new plan.
    $remainingGB = 0;
    $remainingDays = 0;
    $prevPanelData = json_decode($user['Processing_value_three'] ?? '', true);
    if (is_array($prevPanelData)) {
        $prevExpire      = $prevPanelData['expire'] ?? null;
        $prevDataLimit   = $prevPanelData['data_limit'] ?? null;
        $prevUsedTraffic = $prevPanelData['used_traffic'] ?? null;

        $remainingBytes = ($prevDataLimit !== null && $prevUsedTraffic !== null)
            ? max(0, $prevDataLimit - $prevUsedTraffic)
            : 0;
        $remainingGB = $remainingBytes / pow(1024, 3);
        $remainingDays = $prevExpire ? ($prevExpire - time()) / 86400 : 0;
    }

    if ($remainingGB > 0.1 && $remainingDays > 1) {
        $warningText = sprintf(
            "⚠️ <b>هشدار تمدید سرویس</b>\n\nسرویس فعلی شما هنوز حدود <b>%s گیگابایت</b> حجم و <b>%s روز</b> اعتبار باقی‌مانده دارد.\n\nدر صورت ادامه‌ی تمدید، این حجم و زمان باقی‌مانده <b>حذف</b> شده و به سرویس جدید منتقل نخواهد شد.\n\nآیا مایل به ادامه هستید؟",
            round($remainingGB, 2),
            floor($remainingDays)
        );
        $warningKeyboard = json_encode([
            'inline_keyboard' => [
                [
                    ['text' => '✅ تایید و ادامه', 'callback_data' => "confirmextendwarning_" . $codeproduct],
                ],
                [
        ...
        Editmessagetext($from_id, $message_id, $warningText, $warningKeyboard);
        return;
    }
    ...
    // flow exactly as serviceextendselect_ would have without the warning.
    $codeproduct = $dataget[1];
    $nameloc = select("invoice", "*", "username", $user['Processing_value'], "select");
    if ($nameloc == false) {
        sendmessage($from_id, $textbotlang['users']['extend']['error2'], null, 'HTML');
    ...
    // if ($marzban_list_get['type'] == "marzban") {
    //     $current_expire = isset($DataUserOut['expire']) ? $DataUserOut['expire'] : time();
    //     $extension_seconds = intval($product['Service_time']) * 86400;
    //     if ($current_expire > time()) {
    //         $newDate = $current_expire + $extension_seconds;
    //     } else {
    //         $newDate = time() + $extension_seconds;
    //     }
    //     $current_limit = isset($DataUserOut['data_limit']) ? $DataUserOut['data_limit'] : 0;
    //     $new_volume_bytes = intval($product['Volume_constraint']) * pow(1024, 3);
    //     $total_data_limit = $current_limit + $new_volume_bytes;

    //     $datam = array(
    //         "expire" => $newDate,
    //         "data_limit" => $total_data_limit
    //     );
    //     $ManagePanel->Modifyuser($usernamepanel, $nameloc['Service_location'], $datam);
    // }
...
} elseif (preg_match('/confirmchange_([a-zA-Z0-9_.-]+)/', $datain, $dataget)) {
...
} elseif (preg_match('/Extra_volume_([a-zA-Z0-9_.-]+)/', $datain, $dataget)) {
    $username = $dataget[1];
    update("user", "Processing_value", $username, "id", $from_id);
    $textextra = " .";
    ...
    step('home', $from_id);
} elseif (preg_match('/confirmaextra_([a-zA-Z0-9_.-]+)/', $datain, $dataget)) {
    $volume = $dataget[1];
    ...
    if ($user['Balance'] < $price_extra && intval($setting['Extra_volume']) != 0) {
        ...
        update("user", "Processing_value",     $Balance_prim,                                                   "id", $from_id);
        update("user", "Processing_value_one", $nameloc['username'],                                            "id", $from_id);
        update("user", "Processing_value_tow", "extraVolumeafterpay|" . $nameloc['username'] . "|" . $volume,  "id", $from_id);
        sendmessage($from_id, $textbotlang['users']['sell']['None-credit'], $step_payment, 'HTML');
        step('get_step_payment', $from_id);
        return;
    }
...
} elseif (preg_match('/removeserviceuserco-([a-zA-Z0-9_.-]+)/', $datain, $dataget)) {
    $username = $dataget[1];
    $nameloc = select("invoice", "*", "username", $username, "select");
    $marzban_list_get = select("marzban_panel", "*", "name_panel", $nameloc['Service_location'], "select");
...
} elseif (preg_match('/removebyuser-([a-zA-Z0-9_.-]+)/', $datain, $dataget)) {
    $username = $dataget[1];
    $nameloc = select("invoice", "*", "username", $username, "select");
    $marzban_list_get = select("marzban_panel", "*", "name_panel", $nameloc['Service_location'], "select");
...
} elseif (preg_match('/confirmremoveservices-([a-zA-Z0-9_.-]+)/', $datain, $dataget)) {
...
if ($text == $datatextbot['text_account']) {
    ...
    $text_account = sprintf($textbotlang['users']['account'], $first_name, $from_id, $countorder, $user['affiliatescount']);
...
if ($text == $datatextbot['text_Add_Balance'] || $text == "/wallet") {
    ...
    // Show predefined amount buttons instead of immediately asking for
    // typed input. No step is set here - the user must either pick a fixed
    // amount (charge_<amount> callback) or tap "Custom Amount"
    // (custom_amount callback, which arms the `getprice` step) below.
    sendmessage($from_id, sprintf($textbotlang['users']['Balance']['selectAmount'], $Balanceuser), $walletAmountKeyboard, 'HTML');
} elseif ($datain == "custom_amount") {
    // User wants to type a custom amount - fall back to the existing
    // manual-input flow.
    deletemessage($from_id, $message_id);
    $Balanceuser = number_format($user['Balance'], 0);
    sendmessage($from_id, sprintf($textbotlang['users']['Balance']['priceinput'], $Balanceuser), $backuser, 'HTML');
    step('getprice', $from_id);
} elseif (preg_match('/^charge_(\d+)$/', $datain, $chargeAmountMatch)) {
    // User picked one of the predefined amount buttons - save it directly
    // and skip straight to payment method selection.
    deletemessage($from_id, $message_id);
    $chargeAmount = intval($chargeAmountMatch[1]);
    update("user", "Processing_value", $chargeAmount, "id", $from_id);
    sendmessage($from_id, $textbotlang['users']['Balance']['selectPatment'], $step_payment, 'HTML');
    step('get_step_payment', $from_id);
} elseif ($user['step'] == "getprice") {
    // Allow typed amounts with thousands separators (e.g. "100,000") by
    // stripping commas before validating/storing.
    $text = str_replace(',', '', $text);
    if (!is_numeric($text))
        return sendmessage($from_id, $textbotlang['users']['Balance']['errorprice'], null, 'HTML');
    if ($text > 1000000 or $text < 100000)
    ...
    if ($datain == "aqayepardakht") {
        ...
        // if ($user['Processing_value_tow'] == "getconfigafterpay" || str_starts_with($user['Processing_value_tow'], "extendafterpay")) {
        //     $invoice = "{$user['Processing_value_tow']}|{$user['Processing_value_one']}";
        // } else {
        //     $invoice = "0|0";
        // }
        $invoice = buildInvoiceContext($user);
...
if (preg_match('/Confirmpay_user_(\w+)_(\w+)/', $datain, $dataget)) {
    ...
    // if (
    //     $user['Processing_value_tow'] == "getconfigafterpay" ||
    //     str_starts_with($user['Processing_value_tow'], "extendafterpay")
    // ) {
    //     $invoice = "{$user['Processing_value_tow']}|{$user['Processing_value_one']}";
    // } else {
    //     $invoice = "0|0";
    ...
    // }
    $tow = $user['Processing_value_tow'] ?? '';
    $isAfterPayFlow = ($tow !== '' && $tow !== '0' && $tow !== 'none');
    if ($isAfterPayFlow) {
        sendmessage($from_id, $textbotlang['users']['Balance']['Send-receip-buy'], $keyboard, 'HTML');
    } else {
        sendmessage($from_id, $textbotlang['users']['Balance']['Send-receipt'], $keyboard, 'HTML');
