...
// Predefined wallet top-up amounts shown as an inline keyboard when the
// user starts the "Add Balance" flow, instead of immediately prompting for
// typed input. Picking a fixed amount fires callback `charge_<amount>`;
// "Custom Amount" (`custom_amount`) falls back to the existing typed-amount
...
foreach (array_chunk($walletTopupAmounts, 3) as $amountRow) {
    ...
    foreach ($amountRow as $amount) {
        $row[] = [
            ...
            'callback_data' => 'charge_' . $amount,
        ];
    }
    $walletAmountRows[] = $row;
}
$walletAmountRows[] = [
    ['text' => $textbotlang['users']['Balance']['custom_amount_btn'], 'callback_data' => 'custom_amount'],
];
$walletAmountRows[] = [
    ['text' => $textbotlang['users']['backhome'], 'callback_data' => 'backuser'],
];
$walletAmountKeyboard = json_encode([
    'inline_keyboard' => $walletAmountRows,
]);
